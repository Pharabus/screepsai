...
| `upgrader`     | 2 (bootstrap) / 0–6 (miner economy)                       | Bootstrap: `[WORK, CARRY, MOVE]`; Miner: `[WORK×2, CARRY, MOVE]` ×4 (capped at 600 energy below 15k storage, 1100 below 50k, full above) | In miner economy: withdraws from controller container or storage, camps at controller (range 3). At low storage (<5k) a room below RCL 8 keeps 1 upgrader so the controller keeps progressing (body is storage-capped, so it stays small); a built-out RCL 8 room returns 0 to let storage refill. Body cap shrinks under storage scarcity so a 15-WORK upgrader doesn't drain storage faster than miners can refill it. Count scales with storage surplus.                                                                                                                                                                                                                                             |
...
6. upgrader (0–6; at storage < 5k → 1 below RCL 8 / 0 at RCL 8, scales with capacity + storage surplus: >50k/200k/500k = +1/+2/+3)
