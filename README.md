...
| `hauler`       | 2-3/source unlinked, 2 linked (+1 with mineral/lab infra) | `[CARRY×2, MOVE×2]` | Only the nearest hauler to storage responds to urgent spawn/extension/tower energy needs (pulls from storage); other haulers continue normal pickup. Pickup priority: storage link (keeps link pipeline flowing) → dropped resources → full source containers (≥1000 energy) → any source container → mineral container → lab/terminal logistics. Delivery skips controller container when storage is below floor, directing energy to storage first. |
...
- **Haulers** (CARRY + MOVE) pick up dropped energy first, then drain full source containers (≥1000 energy), then the storage link, then remaining containers. On delivery, haulers skip the controller container when storage is below floor — this prevents the upgrader from siphoning energy that storage needs to build reserves.
