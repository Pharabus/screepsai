...
| `hauler`       | 2-3/source unlinked, 1 total linked | `[CARRY×2, MOVE×2]` | Only the nearest hauler to storage responds to urgent spawn/extension/tower energy needs (pulls from storage); other haulers continue normal pickup. Pickup priority: dropped resources → full source containers (≥1000 energy, catches link overflow) → storage link → any source container → mineral container → lab/terminal logistics. Delivery skips controller container when storage is below floor, directing energy to storage first. |
