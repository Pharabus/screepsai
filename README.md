...
| `hauler`       | 2-3/source unlinked, 1 total linked | `[CARRY×2, MOVE×2]` | Only the nearest hauler to storage responds to urgent spawn/extension/tower energy needs (pulls from storage); other haulers continue normal pickup (dropped resources, storage link, source containers) to avoid wasting decaying resources. Picks up minerals from mineral container. Fills lab inputs from storage, collects lab outputs. Moves excess minerals from storage to terminal (above 5k floor). |
...
4. **RCL 4:** Storage is built; haulers stockpile surplus. Upgrader count scales with storage energy: 1 below 10k, 2 below 50k, then capacity-based with bonus tiers at 100k/200k/500k. Extension stamp fills out to 20.
