...
- [ ] **Third room claim** — Score scouted candidates via `evaluateClaim`: require 2 sources, no aggressive neighbours, distance ≤ 3 from W43N58 or W44N57, ideally a different mineral from H. Verify `sendEnergyToColonies` scales to 3 terminals before claiming.
  - **Claim-readiness machinery shipped (prep, not the claim itself):** `findClaimCandidates()` + `claimCandidates()` console command rank all scouted rooms (nearest owned room as prospective home); `scoreClaimTarget` now awards +5 for a mineral none of our owned rooms already mine (the docstring promise is now implemented). Multi-terminal energy balancing verified safe — `sendEnergyToColonies` already ranks N receivers and the per-room loop gives N senders; a forward-looking per-tick receiver-dedupe guard (`_receiversThisTick`) was added for a future empire-logistics model but is **inert today** (one `homeRoom` per colony means two senders can't target the same receiver). **Remaining for the actual claim:** wait for economy to recover (W43N58 storage funding wall backlog), then run `claimCandidates()` → `claim(target)`.
