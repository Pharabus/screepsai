...
- [x] **Min-cut perimeter (terrain-aware barrier) — TRIED & REMOVED** `lessonlearned` — prototyped an Overmind/hivemind-style vertex min-cut (Dinic's, residual-reachability extraction) behind a flag to replace the naive radius ring (which walls parallel to natural walls, e.g. the redundant `x27` column at W43N58). Live validation reversed the rationale: the *sealed* min-cut cost **75 walls vs radius 70** while protecting a smaller area — tightening the protected region to drop x27 drags the barrier to the room edges where edge-ring sealing is expensive, and the headline "70→54" was a leaking (non-sealing) barrier. **Min-cut only wins where natural walls hug the core, not the edges; our geometry favours a compact interior ring.** Removed in v1.0.219 (minCut.ts + the min-cut planner path + the `perimeterMinCut` flag/preview); the radius BFS + the opt-in perimeter overlay (`perimeterVisuals`) remain. Lessons: *measure before optimising* / *live validation is ground truth*. Full implementation in git history v1.0.216–218 if ever worth revisiting against fresh data.
