...
- [ ] **Tunnel-aware remote road planning** `BLOCKED: awaiting decay data` — Teach `placeRemoteRoads` to build roads on natural wall tiles (tunnels) when a tunnel shortcuts a long detour. Confirmed Screeps mechanic: `createConstructionSite(x, y, STRUCTURE_ROAD)` succeeds on wall terrain; build cost is 150× a plain road (45k energy) and the road walks at cost 1 once built. PathFinder treats walls as impassable by default, so the current planner never routes through them and never places the site (e.g. W44N57 source (39,20) sits behind the wall at (37,11) — manually tunnelled 2026-05-25).
  - **Design** (decided): keep the decision *out* of the CostMatrix — movement cost (ticks) and build cost (energy) don't convert at a fixed ratio. Split into (1) **discovery**: two `PathFinder.search` runs per remote source, walls-impassable vs walls-passable-at-cost-1, to find the shortcut tiles and tile savings; (2) **economics**: compare total-route build+repair (all road tiles, both routes — the detour has more tiles, partly offsetting the tunnel's higher per-tile repair) against hauler savings.
  - **Break-even**: `build_break_even_ticks = 45_000 × N_tunnels / (haulers_saved × 1.07 − repair_rate × N_tunnels)`, where 1.07 = hauler body energy (~1600) / TTL (1500). Negative denominator ⇒ never pays off, reject. Gates: build only if break-even < ~50k ticks AND home storage > 100k.
  - **BLOCKER — repair rate is currently a guess.** Earlier estimate of "negligible ~0.01 e/tick" was wrong: if road decay scales with terrain ratio (likely — keeps unmaintained lifetime ~50k ticks across terrains) a wall road loses ~15k hits/1000t ⇒ ~0.15 e/tick passive repair, 150× a plain road. The two hypotheses (scaled vs fixed decay) differ ~150× on repair cost. **Measure the live (37,11) tunnel's `hits` over ~5000 ticks post-build to get the real slope before hard-coding `repair_rate`.** Repair burden lands on the remote room's `remoteBuilder` budget (towers don't reach remotes).
  - Low priority — we have exactly one room (W44N57) that needs this today and it's being handled manually. Revisit only after the decay measurement and when a 2nd/3rd remote with awkward terrain makes it recur.
