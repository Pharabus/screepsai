...
- [x] **Reaction chaining** — `src/utils/reactions.ts` provides `buildReactionChain`, `findNextChainStep`, `chainMissingInputs`, `REACTION_GOALS`. Labs now pick the highest viable step from a priority-ordered goal list (GH2O→XGH2O→XGHO2→… — GH2O/upgrade boost +50% prioritized first because the catalysed X-tier needs catalyst X, which is unbuyable on shard3 at our price cap; X-tier goals remain as aspirational fallbacks), falling back to greedy if no goal chain is achievable.
- [x] **Sticky-lock fix** — `selectReaction` ran `findStickyReaction` *before* the goal loop, so when the input labs held a tier-1 pair (e.g. H+O→OH) it returned OH every tick and never reached goal selection — welding the labs to OH forever (observed live: chain stuck at OH, ghodium precursors unused, O re-bought endlessly). Reordered so goal-directed selection runs first; sticky is now only a fallback to finish a residual batch when no goal step is achievable. `selectReaction` exported for testing.
...
- [x] **Market buying for lab inputs** — `buyForLabs()` in `terminal.ts` runs every 500 ticks. Queries `getChainBuyNeeds()` from labs manager (chain-aware), falls back to active reaction inputs. Buys cheapest sell order ≤ `getMaxBuyPrice()` (shard-aware: 0.5cr default, **150cr on shard3** where the market is inflated) in `BUY_BATCH_SIZE` (3k) batches. Gates on `MIN_BUY_ENERGY_BASE` (30k) for base minerals or `MIN_BUY_ENERGY_INTERMEDIATES` (60k) for compounds, skips own room's mineral type.
  - **Chain-walk fix** — `chainMissingInputs` (reactions.ts) previously returned the *first* gap and early-returned `[]` on the first "producing" step (inputs present, output low). Because `buildReactionChain` linearizes shallow branches first, it short-circuited at the `OH` step (O+H stocked, OH output low) and never surfaced the deep ghodium leaves (U/L/Z/K) or catalyst `X` — so the buyer looped on `O` forever and X-tier boosts could never be supplied. Rewrote it as a whole-chain scan returning *all* leaf inputs (those not produced by any chain step) below `MIN_STEP_AMOUNT`, deduplicated; still returns `[]` when fully stocked. Verified live: real REACTIONS decomposes `UL.ZK=G`, so XGHO2 now surfaces U/L/Z/K/X.
...
- [x] **H-sell deadlock fix** — `sellSurplus` shared `MINERAL_TERMINAL_CEILING` (20k) as its sell trigger with the mineral-miner's combined-stock throttle (also 20k). With 5k pinned in storage the terminal portion topped out ~15k, never crossing its own 20k sell line — H sat unsold at 2 credits. Added `MINERAL_TERMINAL_SELL_FLOOR` (10k); `sellSurplus` now sells terminal mineral above the floor while the throttle stays at the ceiling, so a sellable band always exists. Continuous ~5k sell batches, 15k lab buffer retained.
...
- [x] **NPC-aware scouted-hostile rejection window** — the flat 1500-tick rejection on any scouted hostile over-penalised NPC invaders (cleared by hunters in tens of ticks), winding down remote mining for ~1500 ticks per sighting. Scout now records `RoomMemory.scoutedHostileIsPlayer`; `evaluateRemoteRoom` rejects NPC-only sightings for `NPC_SCOUT_REJECT_TICKS` (300) vs `PLAYER_SCOUT_REJECT_TICKS` (1500) for players, missing-flag→player (fail-safe, preserves legacy behavior exactly). Aggressive-player 20k hard-block untouched. Same NPC-vs-player theme as the `remoteThreat` cooldown split.
...
- [x] **Reclaimed-room cleanup + vault drain** — `cleanupClaimedRoom(room)` (`construction.ts`, every 5 ticks per owned room) `.destroy()`s leftover foreign obstacle structures so they stop blocking movement *and* our RCL structure-count slots. **Root-caused live in W42N59**: a 300-energy leftover spawn filled the single RCL spawn slot → `placeColonySpawn` got `ERR_RCL_NOT_ENOUGH` and silently bailed every tick, builders dumped energy into the controller. Fix destroys obstacle structures even with a little energy; only `LOOTABLE_TYPES` (storage/terminal) **still holding resources** (`storeUsed > 0`) are spared. **Vault mechanic** (live-validated, lesson learned): `.destroy()` *voids* a store, **but `withdraw()` works on a foreign storage/terminal in a room we own** — no WORK part. So the hoard (W42N59's 607k storage) is **drained losslessly by existing haulers** via `pickupForeignStore` (`hauler.ts`), not dismantled. Drain-to-empty → cleanup destroys the empty husk → `placeStorage` builds ours. `myStorage`/`myTerminal` (`src/utils/ownership.ts`) guard every ownership-sensitive `room.storage`/`room.terminal` site so income never flows back into the foreign store. **The earlier dismantle-based `looter` role was built on the untested assumption that `withdraw()` fails on a foreign store — disproven live and removed** (lesson: validate the core mechanic before designing around it).
...
#### Perimeter defense system (completed — v1.0.191–193)

- [x] **BFS perimeter planner** — `src/utils/perimeterPlanner.ts`. Flood-fill from all room border tiles, stopping at a Chebyshev-10 core zone around the spawn. Perimeter = exterior tiles adjacent to the interior boundary. ~80 LOC algorithm, handles arbitrary terrain naturally. Plan stored in `RoomMemory.perimeterPlan`, invalidated by `PERIMETER_PLAN_VERSION` bump or remote-room change.
- [x] **Gate detection** — 2-tile-wide gates toward sources outside the core, controller if distant, and one tile per active remote room exit direction. Gate tiles get ramparts (own creeps can pass); non-gate tiles get walls (impassable, no decay). `getPlannedReserved()` includes non-gate tiles so roads route through gates automatically.
- [x] **RCL-gated build pipeline** — `placePerimeterRamparts` (RCL 5+, gate-only at RCL 6+) and `placePerimeterWalls` (RCL 6+, non-gate only), both gated behind `PERIMETER_STORAGE_MIN` (20k) to prevent energy starvation. Both appended after `placeRamparts` in `runConstruction`, walls before ramparts.
- [x] **Tower wall/rampart repair fix** — `wallRepairMax(room)` now uses `WALL_FLOOR` per-RCL minimums (3=10k, 4=50k, 5=150k, 6=300k, 7=1M, 8=5M) as a floor below the storage-scaled value, preventing paper-thin walls during lean periods. Ramparts co-located on wall tiles are skipped by tower repair (let them decay naturally).
- [x] **Combat event logger** — `src/utils/combatLog.ts`. Ring buffer (100 entries max) in `Memory.combatLog`. Events: `threat_appeared`, `threat_ended`, `safe_mode_activated`, `safe_mode_unavailable`, `tower_energy_low`. Console: `combatLog()` for post-fight review. `replanPerimeter(roomName)` console command for forced recompute.
- [x] **Min-cut perimeter (terrain-aware barrier) — TRIED & REMOVED** `lessonlearned` — prototyped an Overmind/hivemind-style vertex min-cut (Dinic's, residual-reachability extraction) behind a flag to replace the naive radius ring (which walls parallel to natural walls, e.g. the redundant `x27` column at W43N58). Live validation reversed the rationale: the *sealed* min-cut cost **75 walls vs radius 70** while protecting a smaller area — tightening the protected region to drop x27 drags the barrier to the room edges where edge-ring sealing is expensive, and the headline "70→54" was a leaking (non-sealing) barrier. **Min-cut only wins where natural walls hug the core, not the edges; our geometry favours a compact interior ring.** Removed in v1.0.219–220 (minCut.ts + the min-cut planner path + the `perimeterMinCut` flag/preview, and the perimeter RoomVisual overlay + `perimeterVisuals` flag); the radius BFS remains. Lessons: *measure before optimising* / *live validation is ground truth*. Full implementation in git history v1.0.216–218 if ever worth revisiting against fresh data.
...
- [x] **CPU refactor: stop re-scanning every tick** `lessonlearned` — Larger follow-up to the v1.0.229 quick-win `cached()` wrappers (those covered the per-tick `room.find`/`Object.values(Game.creeps)` hot spots in spawner/towers/defense). Three structural items, ~2–4 CPU/tick recoverable, surfaced by a live CPU dive (3 rooms, 43 creeps, measured ~15.5/20 CPU; `rooms`=10.07, `spawner`=2.01, `construction`=1.84 avg EMA). (a) **Spawn-queue caching** — shipped v1.0.246: `buildSpawnQueue` is heap-cached per room for `QUEUE_CACHE_TICKS` (5 ticks) via `_queueCache`; rebuild only when the room is understaffed. (b) **`syncMission` throttling** — subsumed by (a): `syncMission` has exactly one call site (inside `buildSpawnQueue`, confirmed by grep), so the queue cache implicitly throttles it to at most every 5 ticks. No separate code change needed. (c) **`countStructuresAndSites` → tick-cached lookups** — done: `construction.ts` now routes through `getMyStructuresByType` (new `FIND_MY_STRUCTURES`-backed cache in `tickCache.ts`) + `getMySitesByType` (new `FIND_MY_CONSTRUCTION_SITES`-backed cache) so the two per-type `room.find` scans happen at most once per tick. Used `FIND_MY_STRUCTURES` (not `FIND_STRUCTURES`) to preserve exact semantics: foreign-owned structures in reclaimed rooms (e.g. W44N59's previous-owner extensions) must not count against RCL placement caps.
...
- [x] **Upgrader boost dispatch wiring** — Wire `lab.boostCreep()` into the upgrader spawn→dispatch flow for **GH2O (+50% upgrade)** — was XGH2O (+100%) but catalyst X is unbuyable on shard3 at our price cap, so GH2O is the achievable target (revisit XGH2O if an X supply appears). Skip if compound stockpile < 30 × WORK parts. Requires a "boosting" state on spawned upgraders + lab reservation logic so the chosen lab isn't consumed by reactions while a creep is inbound. `src/roles/upgrader.ts`, `src/managers/labs.ts`.
- [x] **Boost application framework** — Designate combat/upgrader creeps to be boosted before departing spawn. Requires filling a lab with the target compound, routing the fresh creep to `lab.boostCreep()` before dispatching to its role. Useful for TOUGH-boosted defenders and WORK-boosted upgraders. Prerequisite for boosted defenders (Phase 3) and boosted keeper killer (Phase 4).
  ...
  - **Step 2 done** — `labs.ts` skips `boostLabId` in the reaction loop; hauler `pickupBoostLab`/`deliverToBoostLab` keep the reserved lab topped up with `boostCompound` (`BOOST_LAB_MINERAL_TARGET` 1500) and energy (`BOOST_LAB_ENERGY_TARGET` 1000). Added `RoomMemory.boostCompound`. Inert until `boostLabId`/`boostCompound` are set.
...
- [x] **CPU bucket throttling with priority/interval** `lessonlearned` — Wrap each profiled manager with a `shouldRun({interval, priority, throttleAt, stopAt})` predicate. When bucket < `throttleAt`, low-priority work slips an interval; below `stopAt` it's skipped entirely. Use a Van der Corput sequence to spread throttled work across ticks instead of bunching. Suggested tiers: below 7k skip visuals/neighbors/remote re-evaluation; below 3k run only defense + spawner + critical hauler dispatch. Pattern from hivemind's `src/hivemind.ts:295-370` and `src/utils/throttle.ts:78-86`. Not urgent at current load — important once we have 2 rooms and 4+ remotes. **Implemented in a prior session**: `src/utils/throttle.ts` `shouldRun` + `THROTTLE_HIGH/NORMAL/LOW` constants; wired in `src/main.ts`.
- [x] **Structures-by-type tick cache** `lessonlearned` — Add `getStructuresByType(room)` to `src/utils/tickCache.ts` returning `Record<StructureConstant, Structure[]>` from a single `room.find(FIND_STRUCTURES)` per tick. Replace ad-hoc filters in `runLinks`, `runTowers`, `runLabs`, `runTerminal`, `runConstruction`, and the roles that scan structures. Pattern from hivemind's `src/prototype/room.structures.ts:97-128` `room.structuresByType[STRUCTURE_LINK]`. **Implemented in a prior session**: `getStructuresByType(room)` exported from `src/utils/tickCache.ts`, used widely across managers and roles.
- [x] **`isOperational` check for RCL-capped structures** `lessonlearned` — Add a helper wrapping `structure.isActive()` with a 500-tick heap cache, used by link selection, tower targeting, and lab planner. Silent killer at RCL 7→6 downgrade and rare overbuild scenarios where structures past the `CONTROLLER_STRUCTURES[type][rcl]` cap stop working. Add when we claim our 2nd room. Pattern from hivemind's `src/prototype/structure.ts:77-104`. **Implemented in a prior session**: `isOperational(structure)` exported from `src/utils/structures.ts`, used in links/labs/towers.
...
- [ ] **Power squad: `powerAttacker` + `powerHealer` + `powerHauler`** — Attacker is pure ATTACK + MOVE; healer sticks on range 1 with HEAL + MOVE; hauler arrives as the bank breaks to scoop the drop and run it home. All three scale bodies to room `energyCapacityAvailable`. **Live near-miss (W44N60, 2026-08-16):** the two-observer setup (W42N59 + W43N58) found a real bank — 2047 power, 2 free tiles — but it cleared every recording threshold by the thinnest margin possible (power 2047 vs 2000 floor, decay 3023 vs 3000 floor) and dropped back out of `scoutedPowerBank` within ~200 ticks of natural decay, before anything could act on it. Confirms the scan pipeline works end-to-end; the gap is purely the missing consumer. Worth considering alongside the squad itself: a lower/no minimum-decay recording floor (or a console alert) so a marginal sighting isn't silently lost before a human even sees it. **Second live sighting, better margin (W40N59, 2026-08-19):** the widened radius-2 highway scan (see above) turned up a genuinely workable target — 8791 power, ~4967 ticks to decay, 3 free adjacent tiles, and only Chebyshev distance 2 from W42N59 (our nearest observer-owning room's neighbor). Comfortably clears every threshold this time, not a last-second near-miss — a good first real target once the squad exists.
- [x] **Power processing loop** — `src/managers/power.ts` `runPowerSpawn()` calls `powerSpawn.processPower()` whenever the spawn holds ≥1 power and ≥`POWER_SPAWN_ENERGY_RATIO` energy, at `THROTTLE_NORMAL`. Hauler logistics (`pickupPowerForSpawn`/`deliverToPowerSpawn` in `hauler.ts`) keep it fed from the terminal, gated behind `POWER_SPAWN_ENERGY_FLOOR` (50k) so it only ever spends genuine surplus. Acquiring power itself is a separate **manual** `buyPower(roomName, amount, maxPricePerUnit?)` console command (not automatic — first-ever trade of this resource, unmeasured market liquidity); `powerStatus()` reports GPL progress + per-room power spawn fill. Live-verified: a 50-power manual buy converted cleanly into GPL progress (0→50/1000) with no manual intervention beyond the purchase.
- [ ] **Invader Core Strongholds** — highway "crossroad" rooms (both coordinates divisible by 10, e.g. W40N60) periodically spawn a fortified NPC outpost: an Invader Core behind ramparts/towers with a Terminal inside holding commodity/resource loot. Same family of highway PvE content as power banks/deposits, but combat-gated rather than decay-gated — needs a real squad to break the ramparts and kill the core before the terminal can be looted, not just a hauler. Nothing scans for these or tracks them yet (`recordHighwayIntel` only records power banks and deposits). Treat as further out on the same risk ladder as `[[project_sk_room_engagement]]` (already paused after live keeper-killer losses) — don't attempt before the power-bank squad above is proven out.
...
- [x] **Add `depositMiner`** — `src/roles/depositMiner.ts`, shipped 2026-08-16. Self-hauling 3-state role (TRAVEL → HARVEST → DELIVER) — no separate dedicated hauler; a deposit's modest yield doesn't justify a second creep and role when one can carry its own harvest home. Abandons its target automatically once the deposit's cooldown escalates past a threshold or the deposit disappears. Manual/operator-triggered via `mineDeposit(homeRoom, depositRoom)` (mirrors `buyPower()`/`claim()`) — deposits are speculative, no automatic empire-wide selection. `mineDeposit()` gates on `Game.map.findRoute()`'s actual walkable hop count, not grid distance — **live incident same day**: a grid-adjacent deposit (`getRoomLinearDistance` = 1) turned out to need an 8-room walkable detour with no direct border exit; the first depositMiner got stuck in a dead-end transit room and runaway stuck-detection repathing dropped the bucket from ~9800 to ~4800 before it was caught and fixed.
- [x] **Feed deposit output into the factory (silicon chain)** — `runFactory` (`factory.ts`) now backward-chains from `wire` (silicon + utrium_bar → wire, utrium_bar from U + energy) via `nextFactoryStep`, taking priority over battery production whenever the room holds any silicon; inert for the other three deposit types (biomass, metal, mist), each of which would need its own independent chain the same way — not built yet, no deposit of those types has been found live so far. Hauler logistics (`pickupFactoryInput`/`deliverToFactoryInputs` in `hauler.ts`) generically read `mem.factoryRecipe` + `COMMODITIES[...].components`, so a future chain slots in with no new hauler code. `utrium_bar` is deliberately excluded from `sellSurplus`'s sellable list (kept as the chain's working intermediate, mirroring how lab reaction intermediates are protected). Live status (2026-08-19): W42N59 has 2000 silicon + ~9.8k U from the W40N58 depositMiner feed, ready to run — currently idle only because the room's own `colonyEnergy` (~5k) sits far below `FACTORY_ENERGY_FLOOR` (120k), the same pre-existing gate that already protects battery production; will engage automatically once that recovers.
...
- [~] **Mission-based architecture** (Harabi §1, `harabi-approach.md`) — migrate to a uniform Manager→Mission→Creep model. **Step 1 done**: `src/utils/missions.ts` now exposes a generic, strictly-typed registry (`MissionRegistry`/`MissionBase`, `getMissionRegistry`, `getMissionsOfType`, registry-wide `garbageCollectMissions`); `RemoteMiningMission` migrated onto it with backward-compatible `type`/`id` backfill — **zero behavior change**. The hard boundary (learned from the shelved hauler pool): missions own spawn quotas/lifecycle/strategic gating, never per-tick creep dispatch. **Step 2 done**: `ColonyState`→`ColonyMission` folded into `Memory.missions.colony` (extends `MissionBase`; `selectedAt`→`createdAt`); `migrateColoniesToMissions()` migrates legacy `Memory.colonies` records once then deletes the key; `getMissionRegistry` backfills sub-maps; behavior preserved, live-verified. **Step 3 done (formalize-only)**: `DefenseMission` (`Memory.missions.defense`) records each owned room's combat engagement — created `active` on threat in `runDefense`, set `retiring` when cleared so the generic GC reclaims it; mirrors the retained `combatActive` flag (safe-mode logging); composition stamped by the spawner. No behavior change. **Deferred**: cross-room defense dispatch (home room sending defenders to a threatened remote/colony) and an `SKMission` type — both carry live-validation risk (no aggressor to test against). Adopt Harabi's coordination rule for all mission code: a mission may read any mission's memory but write only its own.

  Deferred follow-ups (genuinely-new capability, not just formalization — each blocked on a live scenario to validate against; building on unit tests alone cuts against the "live validation is ground truth" lesson):
  - [ ] **Cross-room defense dispatch** — the high-value half of the defense work: a home room dispatches defenders (with `targetRoom`) to a threatened remote/colony, with commit/retreat logic, driven by the existing `DefenseMission` record. Highest priority of the three; revisit the first time a real player threatens a remote (NPC invaders in remotes are already handled by hunters). Combat behavior can't be positively validated live without an aggressor.
  - [ ] **`SKMission` type** — group keeperKiller + miners + haulers for one Source-Keeper room under a single mission with its own lifecycle. Natural once SK mining matures (nascent today). Lower priority.
  - [ ] **Empire-logistics mission** — fold `sendEnergyToColonies`/terminal mineral thresholds into a logistics mission (Harabi §2, bot-level thresholds). Weakest fit per the analysis — mostly terminal sends, not creep coordination; the current per-route hysteresis works. May never be worth it; listed for completeness.
- [ ] **Boost-demand hauler bump** `tunable` — `haulersNeeded(room)` is purely source/distance scaled and insensitive to boost-lab filling demand; the root "all haulers committed to the storage-link drain so the boost lab starves" contention is currently handled by the demand-gated hauler preempt + `ensureBoosted` timeout (v1.0.266), not structurally. A structural fix would be `+1 hauler when upgraderBoostWanted(room)`. **Revisit ONLY if `boostStatus()` shows chronic boost starvation** (a room flagged `⚠ FAILING`) — the preempt already resolves it in practice and an extra creep costs a dispatch slot against the shard3 20-CPU cap (lesson #6). Parked deliberately, not lost.
...
- [~] **Hauler pool (dynamic dispatch)** `lessonlearned` `shelved` — `src/managers/haulerPool.ts` `assignHaulers(room)` (greedy fill+proximity over source containers) + `Memory.haulerPool` flag (default OFF) are implemented and unit-tested, but **shelved dormant — do NOT enable**. Live validation (v1.0.189, W44N57) showed the naive pool conflicts with our task-commitment model: committed haulers ignore their pool assignment (`continueCommittedPickup` wins), and dispatcher-unassigned "extra" haulers fall through to the legacy fullest-first pick — net result was *worse* convergence than legacy (4 haulers on one container, 0 on the other). The code stays behind the off-by-default flag as a no-op (QA-verified). To make it work it needs a **commitment-aware + sticky dispatcher** (pre-deduct already-committed haulers' capacity from their container's need so free haulers route to under-served containers) AND the flag-on path must fully replace the legacy source-container steps so extras don't re-converge. ROI is low at 2 colonies and near-zero on linked rooms (containers stay empty); revisit at 3+ colonies / many unlinked sources. Our commitment-based per-hauler selection is the working approach for now.
