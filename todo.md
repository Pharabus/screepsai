...
- [ ] **Add `depositMiner` + dedicated hauler** — Deposit miners harvest until cooldown climbs, then return; haulers shuttle the resource back to a home-room terminal. Bodies need enough MOVE to handle remote travel at 1:1 fatigue on roadless terrain. **Live confirmation (W43N60, 2026-08-16):** a fresh, never-harvested silicon deposit (`lastCooldown: 0`) sat recorded in `scoutedDeposits` with nothing able to touch it — same "scan works, no consumer" gap as the power squad above.
