...
- [ ] **Add a traffic manager** - Central `runTraffic()` that collects intended move directions from all creeps each tick and resolves conflicts: same-direction creeps yield by priority, opposite-direction creeps swap, stationary creeps on requested tiles get shoved. Would replace the current `ignoreCreeps: true` approach with optimal throughput at chokepoints. Reference: top-tier Screeps bots use intent-based movement systems where creeps register moves and a solver resolves collisions in O(n).
