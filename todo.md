...
- [ ] **Feed deposit output into the factory** — Deposits produce commodity inputs (silicon, biomass, metal, mist), each starting its own independent factory production chain — a chain doesn't need the other three deposit types to produce its own lower-tier goods; only the deepest tier-5 commodities combine components from multiple chains. `runFactory` currently has zero commodity-tier logic (energy→battery only), so right now a mined deposit resource is sellable for credits via the generic `sellSurplus` terminal logic (not a chain intermediate, falls through to the default sell floor) but has no in-game consumption use. Would need factory-level unlocks + new recipe logic from scratch, not a small addition — lower priority than the credits-only path given the pragmatic "achievable over expensive-aspirational" stance.
