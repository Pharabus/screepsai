...
- [ ] **CPU refactor: stop re-scanning every tick** `lessonlearned` — Larger follow-up to the v1.0.229 quick-win `cached()` wrappers (those covered the per-tick `room.find`/`Object.values(Game.creeps)` hot spots in spawner/towers/defense). Three structural items, ~2–4 CPU/tick recoverable, surfaced by a live CPU dive (3 rooms, 43 creeps, measured ~15.5/20 CPU; `rooms`=10.07, `spawner`=2.01, `construction`=1.84 avg EMA). (a) **Spawn-queue caching** — `buildSpawnQueue` rebuilds in full every tick (gated only by `THROTTLE_HIGH`); rebuild on-change instead (heap-cached queue, invalidate when a spawn succeeds or a role count drops), Overmind-style. (b) **`syncMission` throttling** — called per-remote-room inside the spawner loop every tick, each doing an O(creep-count) `Object.values(Game.creeps)` scan (up to ~12×/tick across 3 colonies × remotes); throttle to every 3–5 ticks with mission data held in heap between syncs. (c) **`countStructuresAndSites` → `getStructuresByType`** — `construction.ts` runs 8+ filtered `FIND_MY_STRUCTURES` scans per structure type per 5-tick run; route through the already-cached `getStructuresByType` (~8 call sites). Not urgent (bucket full, 4.5 CPU headroom); revisit before the 4th room or when bucket dips.
