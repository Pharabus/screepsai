...
- [ ] **Market buying for lab inputs** — Labs require two different mineral types to react, but each room only produces one. Add buy logic to `terminal.ts`: identify which base minerals are needed for viable reactions (check `REACTIONS` table), find sell orders on the market via `Game.market.getAllOrders()`, and purchase small batches when the room has energy surplus and terminal capacity. Gate behind having at least 1 lab + terminal + extractor. Without this, labs in single-room setups are permanently idle.
