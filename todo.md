...
- [x] **Reclaimed-room cleanup + vault loot** — `cleanupClaimedRoom(room)` (`construction.ts`, every 5 ticks per owned room) `.destroy()`s leftover foreign obstacle structures so they stop blocking movement *and* our RCL structure-count slots. **Root-caused live in W42N59**: a 300-energy leftover spawn filled the single RCL spawn slot → `placeColonySpawn` got `ERR_RCL_NOT_ENOUGH` and silently bailed every tick, builders dumped energy into the controller. Fix destroys obstacle structures even with a little energy; only `LOOTABLE_TYPES` (storage/terminal) holding `≥ LOOT_MIN_STORE` (10k) are spared. **Loot mechanic** (live-validated): `.destroy()` *voids* a store — only destruction by *damage* drops it. So a real hoard (W42N59's 607k storage) is left for the new **`looter` role** (`src/roles/looter.ts`) to dismantle → drop → existing haulers collect via `pickupLargeDrop`. `looterNeeded` bank-it gate: RCL ≥ 4 + own storage + valid loot target + ≥2 local haulers, so the hoard sits at zero decay until we can bank it. Retired the slow `builder.ts` dismantle hack.
