...
- [x] **Reaction chaining** — `src/utils/reactions.ts` provides `buildReactionChain`, `findNextChainStep`, `chainMissingInputs`, `REACTION_GOALS`. Labs now pick the highest viable step from a priority-ordered goal list (XGH2O→XGHO2→XLHO2→XKHO2→XZHO2 and tier-2 precursors — XGH2O/upgrade boost prioritized first), falling back to greedy if no goal chain is achievable.
