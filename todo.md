...
- [~] **Min-cut perimeter (terrain-aware barrier)** `lessonlearned` — the radius BFS above is naive: a fixed Chebyshev box ignores natural terrain and places walls parallel to natural walls (observed redundant `x27` column at W43N58). Added `src/utils/minCut.ts` (Overmind/hivemind-style vertex min-cut via Dinic's; residual-reachability extraction, NOT edge saturation) + `computeMinCutPerimeter`/`planPerimeter` in `perimeterPlanner.ts`, same `PerimeterPlanData` schema (new `algo` discriminator). **Behind `Memory.perimeterMinCut` (off by default)**; while off, the min-cut candidate is stored in `RoomMemory.perimeterPreview` and drawn green by `runVisuals` for A/B preview. Console `perimeterMinCut(on?)` toggles; `replanPerimeter(room)` prints a count diff. The radius BFS stays as the off-flag default + visual baseline. The min-cut protected region is the core-structure bounding box (+`PROTECT_MARGIN` 2) so it ends before the `x23-25` natural wall band; gate targets use protected-set membership; barrier tiles allowed on the edge-adjacent ring (`BUILD_MIN=1`/`BUILD_MAX=48`) to avoid the x1-lane leak; `PERIMETER_PLAN_VERSION` 3. **SHELVED (v1.0.216) — flag off, radius BFS authoritative.** Live validation on W43N58 reversed the rationale: the *sealed* min-cut is **75 walls vs radius 70** while protecting a smaller area — dropping x27 drags the barrier to the room edges where edge-ring sealing is costly, and the earlier "70→54" was a leaking (non-sealing) barrier. Min-cut only wins where natural walls hug the **core**, not the edges (lesson: measure before optimising / live validation is ground truth). Code is complete + unit-tested + dormant; full rationale in `perimeterPlanner.ts` header. **Revisit:** per-room, for terrain-favourable geometries — flip `perimeterMinCut(true)`, `replanPerimeter(room)`, compare overlay + count diff before adopting.
