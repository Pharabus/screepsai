  ...
  describe('HUNT state', () => {
    ...
    it('targets healer before attacker when a healer is present', () => {
      const attacker = {
        ...
        owner: { username: 'Invader' },
        pos: new (globalThis as any).RoomPosition(24, 25, 'W2N1'),
      };
      const healer = {
        hits: 1000,
        hitsMax: 1000,
        ...
        owner: { username: 'Invader' },
        pos: new (globalThis as any).RoomPosition(26, 25, 'W2N1'),
      };
      const room = mockRoom({
        name: 'W2N1',
        find: vi.fn((type: number, opts?: any) => {
          if (type === FIND_HOSTILE_CREEPS) {
            const filter = opts?.filter ?? (() => true);
            return [attacker, healer].filter(filter);
          }
          return [];
        }),
      });
      ...
      // Must target the healer even though the attacker has lower HP.
      expect(creep.attack).toHaveBeenCalledWith(healer);
    });
