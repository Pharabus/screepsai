...
For cross-room movement, `getPath` uses `maxRooms: 6` and `maxOps: 4000` when the target is in a different room than the creep — enough headroom for diagonal / depth-3 hops (e.g. a scout heading to a SE-diagonal room). The `roomCallback` returns per-room CostMatrix when visibility exists, or `true` for unseen rooms so PathFinder falls back to default terrain costs. Returning `false` here would *skip* the unseen room entirely, blocking any path through unscouted territory. Remote roles path directly to stored source positions via `RoomPosition` constructed from Memory coordinates — no `Game.map.findExit` needed.
