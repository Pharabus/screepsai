...
4. `runSpawner` — calls `ensureRoomPlan(room)` to refresh source/container/miner cache, runs `selectRemoteRooms` every 100 ticks, scans remote rooms via `ensureRemoteRoomPlan`, then **rebuilds the spawn queue per room** via `buildSpawnQueue(room)`. Uses bootstrap economy (harvester-based) until the first source container is detected, then switches to miner economy (miner + hauler + heavy-WORK upgrader). Upgrader count scales with storage reserves: 1 when storage < 15k, 2 when < 50k, then capacity-based (up to 3), with bonus tiers at 100k/200k/500k. The 15k upgrader threshold is deliberately higher than the 10k builder threshold so builders can work without a second upgrader competing for energy. Builder spawning is paused (0 requested) when all sources are linked and storage energy is below `STORAGE_ENERGY_FLOOR` (10k) — in a fully-linked room, builders have no efficient logistics source below this threshold and would fall back to competing with miners at sources. Remote mining roles (scout, remote miners, remote haulers) are appended at lowest priority in miner economy. Defenders are prepended dynamically when threats are active.
