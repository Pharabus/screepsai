...
- **Perimeter defense** (`src/utils/perimeterPlanner.ts`) — two algorithms produce the same `PerimeterPlanData` schema (walls on non-gate tiles, ramparts on gate tiles); `planPerimeter(room)` selects via the `Memory.perimeterMinCut` flag, and `PerimeterPlanData.algo` (`'radius' | 'mincut'`) records which was used (a flag/algo mismatch invalidates the plan in `roomPlanner.ts`).
  - **`computePerimeter` (radius BFS, default/off-flag)** — flood-fill inward from exits, stop at a Chebyshev `CORE_RADIUS` box around the spawn anchor; perimeter = exterior tiles bordering the interior. **Naive flaw it carries:** a fixed-radius ring ignores natural terrain — it walls the radius boundary even where natural walls already block (observed: a redundant `x27` column parallel to a natural wall band at W43N58).
  - **`computeMinCutPerimeter` (min-cut max-flow, `src/utils/minCut.ts`)** — Overmind/hivemind-style vertex min-cut (Dinic's): natural walls are excluded as graph nodes (free pre-existing barrier), so the cut hugs terrain and places the *minimum* ramparts. **Cut extraction is residual-reachability** (a tile is a barrier iff its in-node is source-reachable and its out-node is not) — NOT edge saturation, which would wrongly return every flow-carrying tile (a thick band). Gates: PathFinder routes anchor→each target with barrier tiles passable; the first crossed barrier tile (widened to 2) becomes a gate, with `identifyGateTiles` proximity as a fallback.
  - **Protected region differs by algo.** Radius BFS uses the fixed Chebyshev `CORE_RADIUS` box. Min-cut uses the **core-structure bounding box** (layoutPlan positions ∪ live-built core structures ∪ anchor) expanded by `PROTECT_MARGIN` (2), clamped to buildable range — so the protected area ends *before* a nearby natural wall and the min-cut leans on the wall as free barrier instead of walling a parallel ring outside it (this drops the redundant `x27` column at W43N58). Min-cut barrier tiles may sit on the edge-adjacent ring (`minCut.ts` `BUILD_MIN=1`/`BUILD_MAX=48`) — required, or the cut leaks through the unbuildable-edge x1 lane (seal regression test). Fallback to the radius box when no structures/layoutPlan exist (early rooms; the anchor is always included so the bbox is never truly empty).
  - **⚠️ Min-cut is SHELVED (flag off; radius BFS authoritative)** — full header rationale in `perimeterPlanner.ts`. Live validation on W43N58 showed the *sealed* min-cut costs **75 walls vs radius 70** while protecting a smaller area: dropping x27 drags the barrier to the room edges where edge-ring sealing is expensive. The earlier "−24%" was a leaking (non-sealing) barrier. Min-cut only wins where natural walls hug the **core**, not the edges. Keep it dormant; flip `perimeterMinCut(true)` + `replanPerimeter(room)` and compare per room before adopting.
  - Gate targets: radius BFS gates sources/controller by `chebyshev > CORE_RADIUS`; **min-cut gates them by protected-set membership** (`!protectedSet.has(tile)`) — with the tighter box a source/controller can sit outside the protected region yet within `CORE_RADIUS`, and a radius gate-check would wall it out. Remote-room exit gates are identical for both. `getPlannedReserved()` includes wall tiles so road pathfinding routes through gates. Stored in `RoomMemory.perimeterPlan`.
  - **Preview-first rollout:** while the flag is off, `roomPlanner` stores the min-cut candidate in `RoomMemory.perimeterPreview` (computed only when the overlay is enabled); `runVisuals` overlays it (authoritative plan = walls red / gates blue, preview = green). The overlay is opt-in behind its **own** flag `Memory.perimeterVisuals` (`perimeterVisuals(on?)`), separate from `Memory.visuals` — it's dense/distracting during normal play. Console: `replanPerimeter(room)` recomputes both and prints a tile/wall count diff; `perimeterMinCut(on?)` toggles the algorithm flag. `construction.ts` consumes only the authoritative `perimeterPlan` — never the preview.
