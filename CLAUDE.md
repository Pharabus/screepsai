...
- **Perimeter defense** (`src/utils/perimeterPlanner.ts`) — two algorithms produce the same `PerimeterPlanData` schema (walls on non-gate tiles, ramparts on gate tiles); `planPerimeter(room)` selects via the `Memory.perimeterMinCut` flag, and `PerimeterPlanData.algo` (`'radius' | 'mincut'`) records which was used (a flag/algo mismatch invalidates the plan in `roomPlanner.ts`).
  ...
  - **Preview-first rollout:** while the flag is off, `roomPlanner` also stores the min-cut candidate in `RoomMemory.perimeterPreview`; `runVisuals` overlays it (authoritative plan = walls red / gates blue, preview = green) so it can be eyeballed (`Memory.visuals`) before flipping. Console: `replanPerimeter(room)` recomputes both and prints a tile/wall count diff; `perimeterMinCut(on?)` toggles the flag. `construction.ts` consumes only the authoritative `perimeterPlan` — never the preview.
