...
For cross-room movement, `getPath` uses `maxRooms: 6` and `maxOps: 4000` when the target is in a different room than the creep — enough headroom for diagonal / depth-3 hops (e.g. a scout heading to a SE-diagonal room). The `roomCallback` (`pathRoomCallback` in `trafficManager.ts`) returns per-room CostMatrix when visibility exists, returns `false` (skip) for unseen rooms whose `scoutedOwner` is another player so we don't path creeps into enemy tower range, and returns `true` for all other unseen rooms so PathFinder falls back to default terrain costs. Our own `scoutedOwner` is compared against `Game.spawns[*].owner.username` so a brief vision loss into one of our rooms doesn't make it impassable. Remote roles path directly to stored source positions via `RoomPosition` constructed from Memory coordinates — no `Game.map.findExit` needed.
