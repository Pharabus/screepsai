...
4. `runSpawner` — calls `ensureRoomPlan(room)` to refresh source/container/miner cache, runs `selectRemoteRooms` every 100 ticks, scans remote rooms via `ensureRemoteRoomPlan`, then **rebuilds the spawn queue per room** via `buildSpawnQueue(room)`. Uses bootstrap economy (harvester-based) until the first source container is detected, then switches to miner economy (miner + hauler + heavy-WORK upgrader). Hauler count: base 2 (linked sources add `min(linked,1)`, unlinked add 2–3 per source depending on capacity), +1 when mineral container or labs exist (mineral/lab logistics need dedicated bandwidth). Upgrader count scales with storage reserves: 1 when storage < 15k, 2 when < 50k, then capacity-based (up to 3), with bonus tiers at 100k/200k/500k. The 15k upgrader threshold is deliberately higher than the 10k builder threshold so builders can work without a second upgrader competing for energy. Builder spawning is paused (0 requested) when all sources are linked and storage energy is below `STORAGE_ENERGY_FLOOR` (10k) — in a fully-linked room, builders have no efficient logistics source below this threshold and would fall back to competing with miners at sources. Remote mining roles (scout, remote miners, remote haulers) are appended at lowest priority in miner economy. Defender spawning is prepended dynamically when threats are active, with `defenderComposition(room)` selecting a mix of melee (`defender`), ranged (`rangedDefender`), and `healer` creeps based on threat band and hostile body composition. Creep counts are tracked per-room via `homeRoom` so multi-room colonies don't share quotas.
...
2. `selectRemoteRooms()` (`src/utils/remotePlanner.ts`) evaluates scouted rooms every 100 ticks — rejects owned rooms, rooms reserved by other players (own reservations are accepted), sourceless rooms, rooms with recent hostile sightings (< 1500 ticks old), and rooms where aggressive neighbors have been observed. Picks best rooms by source count up to a storage-gated cap: 1 remote below `REMOTE_ROOM_SCALE_THRESHOLD` (100k storage energy) so a second remote's spawn/bootstrap cost doesn't stall storage growth, 2 once storage clears that bar. Sets `RoomMemory.remoteType` (`'reserved'` for rooms with a controller, `'remote'` for highway/sourceless-controller rooms) and initialises `RoomMemory.defensePolicy` (`'defend'` for reserved, `'flee'` for remote) if not already set.
