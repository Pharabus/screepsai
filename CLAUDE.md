...
- **Hauler pickup priority** (`src/roles/hauler.ts`): urgent responder (storage → spawn/ext when structures need energy, nearest hauler only) → storage link (keeps link pipeline flowing, short trip) → dropped resources (≥50, decay-sensitive) → full source containers (≥1000) → any source container (>0) → mineral container → lab flush/input/output → terminal minerals. The storage link is prioritized early because it's the bottleneck of the link network — backing it up cascades into container overflow and floor drops.
