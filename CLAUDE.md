...
2. `selectRemoteRooms()` (`src/utils/remotePlanner.ts`) evaluates scouted rooms every 100 ticks — rejects owned rooms, rooms reserved by other players (own reservations are accepted), sourceless rooms, and rooms with recent hostile sightings (< 1500 ticks old). Picks the best room(s) by source count and stores them in `RoomMemory.remoteRooms`. The cap auto-scales with home storage: 1 remote below `REMOTE_ROOM_SCALE_THRESHOLD` (100k energy) so bootstrap drain doesn't stall storage growth, 2 remotes once storage clears that bar.
