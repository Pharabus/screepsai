...
4. `runSpawner` — queue is **rebuilt each tick** by `buildSpawnQueue()`, which prepends a dynamic `defender` request when `defendersNeeded(room) > 0`.
...
7. `runConstruction` — one placement per tick (extensions, towers, roads), gated by `MAX_*` RCL tables at the top of `managers/construction.ts`.
...
4. Add a `SpawnRequest` entry — either to `baseSpawnQueue` (always-on minimum) or inside `buildSpawnQueue()` (dynamic / demand-driven, like `defender`).
