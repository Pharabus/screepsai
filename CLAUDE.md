...
- **`.destroy()` voids a structure's store — it does NOT drop the contents.** Loot only drops from destruction by *damage*. So a real hoard (e.g. W42N59's 607k-energy storage) is left standing (recorded in `RoomMemory.lootTargetId`) and the **`looter` role** (`src/roles/looter.ts`, WORK+MOVE) dismantles it to 0 hits → contents drop → existing haulers sweep the pile via `pickupLargeDrop`. The looter is **bank-it gated** (`looterNeeded`): RCL ≥ 4 + a valid loot target + ≥2 local haulers + no looter alive — so the hoard sits at zero decay in the foreign store until we're entitled to a storage. **It deliberately does NOT require our own storage to already exist**: `room.storage` is owner-agnostic and returns the foreign storage occupying the room's single storage slot, so `placeStorage` skips ours and an own-storage gate would deadlock (own storage needs the slot freed → freeing it needs the looter → the looter needed own storage). Looting is the prerequisite: crack at RCL 4, `placeStorage` builds ours on the freed tile next tick, dropped pile banks in (~90% recovered). One-shot: recycles itself (`markIdle`) once the vault is gone.
