...
4. `runSpawner` — calls `ensureRoomPlan(room)` to refresh source/container/miner cache, runs `selectRemoteRooms` every 100 ticks, scans remote rooms via `ensureRemoteRoomPlan`, then **rebuilds the spawn queue per room** via `buildSpawnQueue(room)`. Uses bootstrap economy (harvester-based) until the first source container is detected, then switches to miner economy (miner + hauler + heavy-WORK upgrader). Upgrader count scales with storage reserves: 1 when storage < 10k, 2 when < 50k, then capacity-based (up to 3), with bonus tiers at 100k/200k/500k. Remote mining roles (scout, remote miners, remote haulers) are appended at lowest priority in miner economy. Defenders are prepended dynamically when threats are active.
